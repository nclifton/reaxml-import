...
class ReaxmlImporter
{
    ...
    public function __construct(ReaxmlConfiguration $configuration=null)
    {
        $lang = JFactory::getLanguage();
        $lang->load('lib_reaxml', JPATH_LIBRARIES.DIRECTORY_SEPARATOR.'reaxml', 'en-GB', true);
    ...
    private function sendMail()
    {
        if (($this->xmlErrorCount > 0
                && (!empty($this->configuration->get('done_mail_to')) || !empty($this->configuration->get('error_mail_to'))))

            || (($this->configuration->get('send_success', true) && !empty($this->configuration->get('done_mail_to')))
                && ($this->xmlDoneCount + $this->xmlErrorCount > 0 || $this->configuration->get('send_nofiles', false)))) {

            $status = $this->xmlErrorCount > 0 ? ($this->xmlDoneCount == 0 ? 'FAILURE' : 'ERROR') : 'SUCCESS';
            $mailer = JFactory::getMailer()
                ->setSender(array($this->configuration->get('mail_from_address', JFactory::getConfig()->get('mailfrom')),
                    $this->configuration->get('mail_from_name', JFactory::getConfig()->get('fromname'))))
                ->setSubject(str_replace('{status}', $status,
                    $this->configuration->get('subject', 'REAXML Import Notification: {status}')))
                ->setBody(sprintf(
                    ...
                    , $this->propertyUpdateCount))
                ->addAttachment(
                    $this->configuration->get('log_dir', JPATH_ROOT . '/log') . '/' . self::LOG_NAME . self::LOG_EXT
                    , self::LOG_NAME . self::LOG_EXT);

            $recipients = explode(',', $this->configuration->get('done_mail_to'));
            foreach ($recipients as $recipient) {
                $mailer->addRecipient($recipient);
            }
            if ($this->xmlErrorCount > 0 && !empty($this->configuration->get('error_mail_to'))) {
                $recipients = explode(',', $this->configuration->get('error_mail_to'));
                foreach ($recipients as $recipient) {
                    $mailer->addRecipient($recipient);
                }
            }
