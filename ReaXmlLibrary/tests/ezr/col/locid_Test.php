...
class ReaxmlEzrColLocidTest extends PHPUnit_Framework_TestCase {
	 ...
	 */
	public function getValue_countrynotfoundinxml() {
	
		// Arrange
		$xml = new SimpleXMLElement ( '<residential><address><suburb>Back of Bourke</suburb><state>NSW</state><postcode>2999</postcode></address></residential>' );
		$dbo = $this->getMock ( 'ReaxmlEzrDbo' );
		//$dbo->expects ( $this->once () )->method ( 'lookupEzrLocidUsingLocalityDetails' )->with ( $this->equalTo ( 'Back of Bourke' ) )->willReturn ( false );
