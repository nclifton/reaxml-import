...
class ReaxmlDboLookup_Test extends Reaxml_Tests_DatabaseTestCase {
	...
	public function exists_true() {
		...
		$row = $this->getMock ( 'ReaxmlEzrRow', array (
				'getValue' 
		), array (
				$xml,
				$dbo 
		) );
		$row->expects ( $this->once () )->method ( 'getValue' )->with ( $this->equalTo ( 'mls_id' ) )->willReturn ( 'foo' );
	...
	public function exists_false() {
		...
		$row->expects ( $this->once () )->method ( 'getValue' )->with ( $this->equalTo ( 'mls_id' ) )->willReturn ( 'bar' );
