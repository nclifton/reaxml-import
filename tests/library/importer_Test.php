...
class ReaxmlImporter_Test extends PHPUnit_Framework_TestCase
{
    ...
    public function setup()
    {
        ...
        $this->mailcatcher = new Client(['base_uri'=>'http://127.0.0.1:'.$GLOBALS['MAILCATCHER_HTTP_PORT']]);
    ...
    public function sends_mail_nofiles()
    {
        ...
        // arrange
        $configuration = $this->getMockBuilder('ReaxmlConfiguration')->getMock();
        ...
        $configuration->expects(
            ...
            ->withConsecutive(
                [$this->equalTo('send_success'), $this->equalTo(true)],
                [$this->equalTo('done_mail_to')],
                [$this->equalTo('send_nofiles'), $this->equalTo(false)],
                [$this->equalTo('mail_from_address'), $this->equalTo('website@reaxmltest')],
                [$this->equalTo('mail_from_name'), $this->equalTo('REAXML TEST SITE')],
                [$this->equalTo('subject'), $this->equalTo('REAXML Import Notification: {status}')],
                [$this->equalTo('log_dir'), $this->equalTo(JPATH_ROOT . '/log')],
                [$this->equalTo('done_mail_to')])
        ...
        $dbo = $this->getMockBuilder('ReaxmlEzrDbo')->getMock();

        $importer = new ReaxmlImporter($configuration);
        $importer->setDbo($dbo);
    ...
    public function cleanDirectories()
    {
        ...
        $doneDir = __DIR__ . DIRECTORY_SEPARATOR . 'test_done';
        $workDir = __DIR__ . DIRECTORY_SEPARATOR . 'test_work';
        $inputDir = __DIR__ . DIRECTORY_SEPARATOR . 'test_input';
        $errorDir = __DIR__ . DIRECTORY_SEPARATOR . 'test_error';
        $logDir = __DIR__ . DIRECTORY_SEPARATOR . 'test_log';

        if(!file_exists($doneDir))
            mkdir($doneDir);
        ...
        $this->recursiveUnlink($doneDir . DIRECTORY_SEPARATOR . '*');
        $this->recursiveUnlink($workDir . DIRECTORY_SEPARATOR . '*');
        $this->recursiveUnlink($inputDir . DIRECTORY_SEPARATOR . '*');
        $this->recursiveUnlink($errorDir . DIRECTORY_SEPARATOR . '*');
        $this->recursiveUnlink($logDir . DIRECTORY_SEPARATOR . '*');
