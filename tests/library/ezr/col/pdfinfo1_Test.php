...
class ReaxmlEzrColPdfinfo1_Test extends PHPUnit_Framework_TestCase {
	private $file;
	...
	public function getValue_isnew_notfound_givesempty() {
		...
		// Act
		$col = new ReaxmlEzrColPdfinfo1 ( new ReaxmlEzrRow ( $xml, $dbo ) );
		$value = $col->getValue ();
	...
	public function getValue_file_copiesfile_givesfilename() {
		// Arrange
		$xml = new SimpleXMLElement ( '<residential><uniqueID>foo</uniqueID><objects><document file="hello.pdf" format="pdf" id="1" /></objects></residential>' );
		
		$configuration = new ReaxmlConfiguration ();
		$configuration->work_dir = __DIR__ . '/../../files';
		
		// Act
		$col = new ReaxmlEzrColPdfinfo1 ( new ReaxmlEzrRow ( $xml, null, $configuration ) );
		$value = $col->getValue ();
	...
	public function getValue_url_copiesfile_givesfilename() {
		// Arrange
		$xml = new SimpleXMLElement ( '<residential><uniqueID>foo</uniqueID><objects><document url="http://www.newforests.net/wp-content/uploads/2011/01/sample_pdf.pdf" format="pdf" id="1" /></objects></residential>' );
		
		// Act
		$col = new ReaxmlEzrColPdfinfo1 ( new ReaxmlEzrRow ( $xml ) );
		$value = $col->getValue ();
	...
	public function getValue_idonly_isnew_givesempty() {
		// Arrange
		$xml = new SimpleXMLElement ( '<residential><uniqueID>foo</uniqueID><objects><document id="1" /></objects></residential>' );
		$dbo = $this->createMock ( 'ReaxmlEzrDbo' );
		$dbo->expects ( $this->once () )->method ( 'exists' )->with ( $this->equalTo ( 'foo' ) )->willReturn ( false );
	...
	public function getValue_idonly_isnotnew_looksupfilenameanddeletesit_returnsempty() {
		...
		$dbo = $this->createMock ( 'ReaxmlEzrDbo' );
		$dbo->expects ( $this->once () )->method ( 'exists' )->with ( $this->equalTo ( 'foo' ) )->willReturn ( true );
		$dbo->expects ( $this->once () )->method ( 'lookupEzrPdfinfo1' )->with ( $this->equalTo ( 'foo' ) )->willReturn ( 'hello.pdf' );
