...
class folderbrowser_Test extends reaxml_selenium_TestCase {
	...
	public function displays_folders() {
        ...
        $this->adminLogin ();
        $this->url ( 'http://'.$GLOBALS ['SERVER_NAME'].'/administrator/index.php?option=com_reaxmlimport&controller=config&view=folderbrowser&tmpl=component&inputid=jform_input_dir&urlinputid=jform_input_url&folder='.urlencode ('/var/www/reaxml/ftp') );
		...
		$this->assertThat ( $this->byCssSelector ( 'form[name="adminForm"] input[name="folder"]' )->attribute('value'), $this->equalTo ('/var/www/reaxml/ftp'));
	...
	public function can_use_current_folder() {
		...
		$this->assertThat($this->byId('folder')->attribute('value'), $this->equalTo('/var/www/reaxml'));
		...
		$this->assertThat($this->byId('jform_input_dir')->attribute('value'), $this->equalTo('/var/www/reaxml'));
	...
	public function uses_root_folder_if_supplied_folder_does_not_exist() {
		...
		$this->assertThat ( $this->byCssSelector ( 'form[name="adminForm"] input[name="folder"]' )->attribute('value'), $this->equalTo ('/var/www/reaxml') );
	...
	public function can_go_to_another_folder() {
		...
		$this->assertThat ( $this->byCssSelector ( 'form[name="adminForm"] input[name="folder"]' )->attribute('type'), $this->equalTo ('text') );
		$this->assertThat ( $this->byCssSelector ( 'form[name="adminForm"] input[name="folder"]' )->attribute('value'), $this->equalTo ('/var/www/reaxml/ftp/input') );

        $path = '/var/www/reaxml/ftp/input';
        $folders = explode('/',$path);
        $items = $this->elements($this->using('css selector')->value('ul.breadcrumb li'));
	...
	public function can_drill_down_to_a_subfolder() {
		...
		$this->url ( 'http://'.$GLOBALS ['SERVER_NAME'].'/administrator/index.php?option=com_reaxmlimport&controller=config&view=folderbrowser&tmpl=component&inputid=jform_input_dir&urlinputid=jform_input_url&folder='.urlencode ('/var/www/reaxml') );
		$this->byCssSelector('ul.folders li.item9 a')->click();
