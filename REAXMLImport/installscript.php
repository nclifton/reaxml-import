...
class com_reaxmlImportInstallerScript {
	...
	function postflight($type, $parent) {
		...
		$srcdir = JPATH_ADMINISTRATOR . DS . 'components' . DS . 'com_reaxmlimport' . DS . 'cli';
		$destdir = JPATH_SITE . DS . 'cli';
		if (! file_exists ( $destdir )) {
			mkdir ( $destdir );
		}
		if ($handle = opendir ( $srcdir )) {
			while ( false !== ($fileName = readdir ( $handle )) ) {
				if (is_file ( $srcdir . DS . $fileName )) {
					rename ( $srcdir . DS . $fileName, $destdir . DS . $fileName );
				}
			}
			closedir ( $handle );
		}
